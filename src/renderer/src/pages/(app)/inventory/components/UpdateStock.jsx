...
export default function UpdateStock() {
  ...
  const { createItem: createLog } = useCollection('snacks_logs');
  ...
  const handleQuantityUpdate = async (e) => {
    ...
    if (!quantityUpdate.amount || isNaN(quantityUpdate.amount) || quantityUpdate.amount <= 0) {
      await window.api.notify('Error', 'Please enter a valid quantity');
      return;
    }
    ...
    try {
      ...
      await window.api.notify('Success', `Stock quantity ${quantityUpdate.action === 'add' ? 'increased' : 'decreased'} successfully!`);
    } catch (error) {
      console.error("Error updating quantity:", error);
      // await window.api.notify('Error', 'Error updating stock quantity');
  ...
  const handlePriceUpdate = async (e) => {
    ...
    if (!priceUpdate.newPrice || isNaN(priceUpdate.newPrice) || priceUpdate.newPrice < 0) {
      await window.api.notify('Error', 'Please enter a valid price');
      return;
    }
    ...
    try {
      ...
      await window.api.notify('Success', 'Price updated successfully!');
    } catch (error) {
      console.error("Error updating price:", error);
      await window.api.notify('Error', 'Error updating price');
    } finally {
      setIsLoading(false);
      handleClose();
    }
  ...
  const handleLocationUpdate = async (e) => {
    ...
    if (locationUpdate.newLocation === formData.location) {
      await window.api.notify('Info', 'Location is unchanged');
      return;
    }
    ...
    try {
      ...
      await window.api.notify('Success', 'Location updated successfully!');
    } catch (error) {
      console.error("Error updating location:", error);
      await window.api.notify('Error', 'Error updating location');
    } finally {
      setIsLoading(false);
      handleClose();
    }
